...
(() => __awaiter(this, void 0, void 0, function* () {
    ...
    const badgePrimary = '<span class="badge success"><div class="dot"></div>Primary</span>', badgeSecondary = '<span class="badge generic"><div class="dot"></div>Secondary</span>', badgeContact = '<span class="badge warning"><div class="dot"></div>Contact</span>', badgeUnverified = '<span class="badge alert"><div class="dot"></div>Unverified</span>', loadingSpinner = '<div class="loading-container"><div class="loading"><span class="loader"></span></div></div>';
    ...
    d.getElementById('handler').addEventListener('input', (e) => {
        ...
        for (const c of value) {
            if (!RSUtils.regex.handler.test(c))
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (password.length > 0) {
                    res = yield fetch(`${endpoint}/security`, {
                        'method': 'POST',
                ...
                if (!validPassword) {
                    ...
                    _form.addEventListener('submit', (ev) => __awaiter(this, void 0, void 0, function* () {
                        ...
                        yield loadSecuritySection();
                    }));
                    securitySection.append(_form);
                    if (password.length > 0)
                        RSNotifications.create(msg, 'error');
                    return;
                }
                // #endregion
                // #region Content
                ...
                for (const email of data.emails) {
                    ...
                    switch (email.type) {
                        case 0:
                            _email.innerHTML += ' ' + badgePrimary;
                            break;
                        case 1:
                            _email.innerHTML += ' ' + badgeContact;
                            break;
                        case 2:
                            _email.innerHTML += ' ' + badgeSecondary;
                            break;
                    }
                    if (!email.verified)
                        _email.innerHTML += badgeUnverified;
                    _head.append(_email);
                    // #endregion
                    // #region Footer
                    ...
                    if (!email.verified) {
                        ...
                        _verify.addEventListener('click', (ev) => __awaiter(this, void 0, void 0, function* () {
                            ev.preventDefault();
                            ev.stopPropagation();
                            if (email.verified)
                                return;
                            try {
                                const res = yield fetch(`${endpoint}/email/send-verification`, {
                                    'method': 'POST',
                    ...
                    if (email.type !== 0) {
                        ...
                        _delete.addEventListener('click', (ev) => __awaiter(this, void 0, void 0, function* () {
                            ev.preventDefault();
                            ev.stopPropagation();
                            if (email.type === 0)
                                return;
                            try {
                                const res = yield fetch(`${endpoint}/email/delete`, {
                                    'method': 'POST',
                                    'body': new URLSearchParams({ '_csrf': csrfToken, 'email': email.id }),
                                    'headers': { 'Authorization': authToken }
                                });
                    ...
                    form.addEventListener('submit', (ev) => __awaiter(this, void 0, void 0, function* () {
                        ...
                        try {
                            if (onSubmit)
                                return;
                            onSubmit = true;
                            const res = yield fetch(`${endpoint}/email`, {
                                method: 'PUT',
                                body: new URLSearchParams(data),
                                headers: { 'Authorization': authToken }
                            });
                            const _data = yield res.json();
                            onSubmit = false;
                    ...
                    form.addEventListener('submit', (ev) => __awaiter(this, void 0, void 0, function* () {
                        ...
                        try {
                            if (onSubmit)
                                return;
                            onSubmit = true;
                            const res = yield fetch(`${endpoint}/email/set`, {
                                method: 'POST',
                                body: new URLSearchParams(data),
                                headers: { 'Authorization': authToken }
                            });
                ...
                if (!data._2fa) {
                    ...
                    button.addEventListener('click', () => __awaiter(this, void 0, void 0, function* () {
                        try {
                            if (qr === '') {
                                const res = yield fetch(`${endpoint}/security/2fa/enable`, {
                                    method: 'POST',
                    ...
                    button.addEventListener('click', () => __awaiter(this, void 0, void 0, function* () {
                        ...
                        button.addEventListener('click', () => __awaiter(this, void 0, void 0, function* () {
                            try {
                                const res = yield fetch(`${endpoint}/security/2fa/disable`, {
                                    method: 'POST',
                                    body: new URLSearchParams({
                                        _csrf: csrfToken
                                    }),
    ...
    d.querySelectorAll('[data-type="session-revoke"]').forEach((el) => {
        el.addEventListener('click', (ev) => __awaiter(this, void 0, void 0, function* () {
            ...
            ev.stopPropagation();
            try {
                const res = yield fetch(`${endpoint}/session/delete`, {
                    'method': 'POST',
                    'body': new URLSearchParams({ '_csrf': csrfToken, 'session': el.dataset.id })
                });
                const data = yield res.json();
                if (res.status === 200)
                    el.closest('div[class="session"]').remove();
                RSNotifications.create(data.message, res.status === 200 ? 'success' : 'error');
            }
            catch (e) {
                console.error(e);
        ...
        form.addEventListener('submit', (ev) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                switch (form.dataset.type) {
                    case 'profile':
                        const res = yield fetch(`${endpoint}/profile`, {
                            method: 'POST',
                            body: new URLSearchParams(data)
                        });
                        if (res.status !== 200) {
