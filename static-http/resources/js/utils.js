...
class RSUtils {
    ...
    static sanitizeUrl(str) {
        ...
        if (__blacklist.some(p => str.toLowerCase().startsWith(p)))
            str = 'https://youtu.be/hiRacdl02w4?t=46';
        if (str.startsWith('//'))
            str = 'https:' + str;
...
class RSParallax {
    static parse() {
        d.querySelectorAll('img-parallax').forEach((e) => {
            const content = e.innerHTML, src = e.getAttribute('src'), delay = e.getAttribute('delay'), style = e.getAttribute('style'), defStyle = `background-image: url('${src}'); background-size: cover; background-repeat: no-repeat;`;
...
class RSNotifications {
    ...
    static create(message, type, timeout = 5000) {
        const notf = d.createElement('div'), notfId = RSNotifications.notfId++;
