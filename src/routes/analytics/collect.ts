...
import { logger, conf } from '../../globals';
...
router.post('/', async (req, res, next) => {
	try {
		...
		if (dnt === '1' && conf.production) return res.status(403).json({ message: 'DNT is enabled.', dnt: true });
		...
		if (!RSMisc.ValidURL(body.path)) return next(httpError(400, 'Invalid path.'));
		
		if (body.referrer) {
			if (typeof body.referrer !== 'string') return next(httpError(400, 'Invalid referrer.'));
			if (!RSMisc.ValidURL(body.referrer)) return next(httpError(400, 'Invalid referrer.'));

			const referrer = new URL(body.referrer);
			if (referrer.hostname === conf.domain) return res.status(200).json({ message: 'IGNORED' });
		...
		try {
			...
			await Analytics.SetVisit(body.timezone, href.pathname, body.screen, body.referrer, RSMisc.AnonymizeAgent(req.useragent.source));
		} catch (e) {
			logger.error(e);
