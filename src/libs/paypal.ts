...
const base = conf.production ? "https://api-m.paypal.com" : "https://api-m.sandbox.paypal.com";
...
export async function generateAccessToken(): Promise<string> {
	try {
		const auth = Buffer.from(`${conf.paypal.id}:${conf.paypal.secret}`).toString('base64');
