...
import { RSCrypto, RSError, RSMisc, RSRandom, RSTime } from '../libs/RSEngine';
...
}, logger, conf.production ? Mailer.Mode.Internal : Mailer.Mode.Debug, conf.root, pgConn, conf.keys.MASTER);
...
export class Email {
	...
	async GetUser(): Promise<User|null> {
		return new Promise(async (resolve, reject) => {
			...
			reject(new RSError('User not found', 404));
		});
	}
	...
	public static async Validate(email: string): Promise<boolean> {
		...
		try {
			...
			const data = (await readFile(path.join(process.cwd(), 'disposable_email_blocklist.conf'), 'ascii')).replace('\r\n', '\n');
	...
	public static HMAC(email: string): string { return RSCrypto.HMAC(email + conf.keys.SALT, conf.keys.HMAC); }
	...
	public async Send(type: Email.MailType, ...args: string[]) {
		...
		function verifyLink(token: string) { return `${conf.root}/verify-email/${token}`; }
		const contactLink = `${conf.root}/contact`;
		...
		try {
			...
			switch (type) {
				case Email.MailType.NEW_USER: {
					...
					const valHash = RSCrypto.HMAC(validator, conf.keys.HMAC);
				...
				case Email.MailType.PASSWORD_RESET_REQUEST: {
					...
					body = `<h2>Reset your password.</h2>
						...
						<p><a href="${conf.root}/accounts/change-password?token=${token}">${conf.root}/accounts/change-password?token=${token}</a>
...
export class Token {
	...
	public static HMAC(secret: string): string { return RSCrypto.HMAC(secret, conf.keys.HMAC); }
...
export class UserToken extends Token {
	...
	public static async Set(uid: string, remember: boolean, useragent: string, verified: boolean): Promise<UserToken.Response | null> {
		...
		try {
			const id = RSCrypto.RandomBytes(16);
			const validator = RSCrypto.RandomBytes(32);
			const browser = RSMisc.AnonymizeAgent(useragent);
			...
			const _res = await client.query(`INSERT INTO auth_tokens (id, usrid, client, val_key, is_temp, expires_at, verified) VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP + INTERVAL '1 ${remember ? 'WEEK' : 'HOUR'}', $6) RETURNING created_at, expires_at, last_usage, last_update`, [
				...
				uid,
				browser,
				RSCrypto.HMAC(validator, conf.keys.HMAC),
				!remember,
				verified
			]);
			...
			return {
				token: new UserToken(id, RSCrypto.HMAC(validator, conf.keys.HMAC), _res.rows[0].created_at, _res.rows[0].expires_at, browser, uid, _res.rows[0].last_usage, _res.rows[0].lastUpdate, !remember, verified),
				text: `${id}.${validator}`
			}
		} catch (e) {
			logger.error(e);
			...
			try {
				...
				if (tok.lastUpdate.getTime() < Date.now() - RSTime._HOUR_) {
					...
					await client.query(`UPDATE auth_tokens SET val_key = $1, last_update = CURRENT_TIMESTAMP, client = $2 WHERE id = $3`, [
						...
						RSMisc.AnonymizeAgent(userAgent),
						id
					]);
	...
	public GetCookieParams(isOnion: boolean): CookieOptions {
		return {
			...
			path: '/',
			secure: conf.production && !isOnion,
			httpOnly: true,
			sameSite: 'lax',
			domain: isOnion ? undefined : (conf.production ? `.${conf.domain}` : 'localhost')
		};
	}
	...
	public async GenerateCSRF(): Promise<string> {
		...
		try {
			await client.query(`UPDATE auth_tokens SET _csrf = $1 WHERE id = $2`, [ RSCrypto.HMAC(csrf, conf.keys.HMAC), this.id ]);
	...
	public async ValidateCSRF(csrf: string): Promise<boolean> {
		...
		try {
			...
			return RSCrypto.Compare(_res.rows[0]._csrf, RSCrypto.HMAC(csrf, conf.keys.HMAC));
		} catch (e) {
			logger.error(e);
		} finally {
			client.release();
	...
	public async GenerateConfigAuth(): Promise<string> {
		...
		try {
			await client.query(`UPDATE auth_tokens SET _config_auth = $1 WHERE id = $2`, [ RSCrypto.HMAC(auth, conf.keys.HMAC), this.id ]);
	...
	public async ValidateConfigAuth(auth: string): Promise<boolean> {
		...
		try {
			...
			return RSCrypto.Compare(_res.rows[0]._config_auth, RSCrypto.HMAC(auth, conf.keys.HMAC));
		} catch (e) {
			logger.error(e);
		} finally {
			client.release();
...
export class User {
	...
	private static async GenerateCryptoKey(hash: string): Promise<string> {
		return await RSCrypto.PBKDF2(conf.keys.MASTER, hash, 1000, 32);
	}
	...
	public async GenerateRecoveryCodes(): Promise<string[]> {
		...
		try {
			...
			for (var i = 0; i < 3; i++) {
				...
				codes.push(code);
				hashes.push(RSCrypto.HMAC(code, conf.keys.HMAC));
			}
	...
	public async VerifyRecoveryCode(code: string): Promise<boolean> {
		...
		try {
			...
			const c = RSCrypto.HMAC(code, conf.keys.HMAC);
			var found = false;
