...
router.get('/:handler', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (user.bio) {
        const bio = RSEngine_1.RSUtils.EscapeHtml(user.bio.replace(/\n/g, ' ').substring(0, 200));
    ...
    res.rs.html.body = `<input type="hidden" id="user-handler" value="${user.handler}">
		...
		<div class="profile-container">
			<div class="panel left">
				<img src="${user.avatar}" class="avatar" alt="${RSEngine_1.RSUtils.EscapeHtml(user.name)}'s avatar" width="260" height="260"><br>
				<div class="profile-info">
					<div class="txt-ellipsis">
						<span style="font-size: 25px">${RSEngine_1.RSUtils.EscapeHtml(user.name)}</span><br>
					</div>
					<span style="font-size: 15px; color: #999">@${RSEngine_1.RSUtils.EscapeHtml(user.handler)}</span><br>
			...
			<div class="panel right">
				<div class="container">
					${user.bio ? `<div class="user-text" id="user-bio">Loading...<div style="display: none">${RSEngine_1.RSUtils.EscapeHtml(user.bio)}</div></div>
