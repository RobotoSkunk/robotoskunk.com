...
(() => __awaiter(void 0, void 0, void 0, function* () {
    ...
    app.use((req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
        const nonce = crypto_1.default.randomBytes(24).toString('base64url'), today = new Date(), url = `${req.protocol}://${req.hostname}${req.originalUrl}`, onlyUrl = url.split('?')[0], authToken = req.cookies.auth_token || '', lang = (0, lang_1.getCode)(req.cookies.lang || req.headers['accept-language'] || 'en');
        var tokenData;
        ...
        res.renderDefault = (view = 'layout.ejs', options = {}) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            if (options.useZxcvbn) {
                const zxcvbn = yield ejs_1.default.renderFile(path_1.default.join(process.cwd(), '/layouts/utils/zxcvbn.ejs'), { nonce: res.rs.server.nonce, version: res.rs.conf.version });
        ...
        res.header('Content-Security-Policy', `default-src 'self' 'unsafe-hashes' 'unsafe-inline' ${!globals_1.conf.production ? 'localhost:*' : ''} www.redbubble.com *.robotoskunk.com robotoskunk.com www.youtube.com *.paypal.com *.facebook.com ko-fi.com *.ko-fi.com cdnjs.cloudflare.com api.pwnedpasswords.com js.hcaptcha.com *.hcaptcha.com translate.googleapis.com;`
            + `script-src 'strict-dynamic' 'unsafe-inline' https: ${!globals_1.conf.production || isOnion ? 'http:' : ''} 'nonce-${nonce}';`
            + `base-uri 'self';`
            + `object-src 'none';`
    ...
    function HandleErrors(err, req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            res.rs.error = {
                ...
                'debug': (globals_1.conf.production ? undefined : (err.message ? err.message.replace(/\\/gm, '\\\\').replace(/\`/gm, '\\`') : undefined)),
                'imgAlt': 'Alex Skunk dizzy on the floor',
            };
            switch (code) {
                case 400:
                    res.rs.error.message = RSEngine_1.RSRandom.Choose(['Bad request.', "I can't interpret that.", 'What kind of request is that?', 'This request is very confusing.']);
                    break;
                case 401:
                    res.rs.error.message = RSEngine_1.RSRandom.Choose(["You must authenticate.", "Authentication missing.", "You haven't authenticated.", "Authentication required."]);
                    break;
                case 403:
                    res.rs.error.message = RSEngine_1.RSRandom.Choose(["You can't be here.", "Get out of here.", "You don't have permissions to be here.", "Access denied.", "Why you're here?", "Stop! You can't be here.",
                    ...
                    break;
                case 404:
                    res.rs.error.message = RSEngine_1.RSRandom.Choose(["What you are looking for was not found.", "Are you lost?", "That page doesn't exists.", "What? What was you doing?", "Oops! Something is broken.",
                    ...
                    break;
                case 408:
                    res.rs.error.message = RSEngine_1.RSRandom.Choose(["Request timed out.", "Request took too long.", "Request took too long to respond.", "Request took too long to process."]);
                ...
                case 418:
                    ...
                    break;
                case 429:
                    res.rs.error.message = RSEngine_1.RSRandom.Choose(["You're doing that too much.", "You're doing that too fast.", "You're doing that too often.", "You're doing that too much, too fast, too often.",
                        'Slow down.', 'Woah! Slow down cowboy!', 'Dude, slow down!', 'Too many requests!', 'Slow down buddy!']);
                    break;
                case 500:
                    res.rs.error.message = RSEngine_1.RSRandom.Choose(["Beep boop... beeeeep...... boop...", "Something went wrong...", "Server error.", "Oops! Something went wrong.", "This is not your fault.",
    ...
    setInterval(() => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            yield client.query('DELETE FROM tokens WHERE expires_at < NOW()');
            yield client.query(`DELETE FROM auth_tokens WHERE created_at < NOW() - INTERVAL '1 HOUR' AND verified = false`);
            yield client.query(`DELETE FROM csp_reports WHERE created_at < NOW() - INTERVAL '1 MONTH'`);
            yield client.query(`DELETE FROM users WHERE end_date < NOW()`);
            yield client.query(`DELETE FROM users U USING emails E WHERE E.refer = 0 AND E.verified = false AND E.usrid = U.id AND U.created_at < NOW() - INTERVAL '1 HOUR'`);
            yield client.query(`DELETE FROM emails WHERE verified = false AND created_at < NOW() - INTERVAL '1 HOUR'`);
            yield client.query(`DELETE FROM blacklist WHERE ends_at < NOW()`);
        }
        catch (e) {
            globals_1.logger.error(e);
        }
