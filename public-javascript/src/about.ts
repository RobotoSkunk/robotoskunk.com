...
(() =>
{
	const data: {node: HTMLElement, distance: number, dir: string}[] =
	[
	...
	function updateAnimations(): void
	{
		...
		for (var i in data) {
			var x = RSUtils.clamp(
				w.scrollY - (data[i].node.offsetTop - (w.innerHeight / 3 * 2)),
				0,
				data[i].distance
			);
			...
			data[i].node.style.opacity = RSUtils.clamp(x / data[i].distance, 0, 1).toString();
		}
	}
